...
export function ArticleDetailPage({ article, onClose, onLike, onBookmark }: ArticleDetailPageProps) {
  ...
  return (
    <div className={`fixed inset-0 z-40 bg-[var(--color-cloud)] flex flex-col ${exiting ? "animate-window-exit" : "animate-window-enter"}`}>
      {/* Top bar */}
      <div className="flex items-center justify-between px-4 py-3 bg-[var(--color-cloud)]/90 backdrop-blur-lg border-b border-[var(--color-paper-deep)] flex-shrink-0">
        <button onClick={handleClose} className="p-1 cursor-pointer text-[var(--color-ink)] hover:text-[var(--color-ink-soft)] transition-colors">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <line x1="19" y1="12" x2="5" y2="12" />
            <polyline points="12 19 5 12 12 5" />
          </svg>
        </button>
        <h1 className="text-sm font-medium text-[var(--color-ink)] truncate mx-3 flex-1 text-center">{article.title}</h1>
        <button onClick={handleBookmark} className={`p-1 cursor-pointer transition-colors ${bookmarked ? "text-[#F5A623]" : "text-[var(--color-ink-ghost)]"}`}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-5 pt-5 pb-24">
          {/* Title */}
          <h1 className="text-[22px] font-bold text-[var(--color-ink)] leading-tight mb-4">{article.title}</h1>

          {/* Author */}
          <div className="flex items-center gap-3 mb-5">
            <img src={article.author.avatarUrl} alt={article.author.name} className="w-9 h-9 rounded-full bg-[var(--color-paper-warm)]" />
            <div className="flex flex-col">
              <span className="text-sm font-medium text-[var(--color-ink)]">{article.author.name}</span>
              <span className="text-xs text-[var(--color-ink-ghost)]">{formatDate(article.createdAt)} · {article.readTime} min read</span>
            </div>
          </div>

          {/* Summary callout */}
          <div className="bg-[var(--color-paper-warm)] rounded-xl p-4 mb-6">
            <p className="text-sm text-[var(--color-ink-soft)] leading-relaxed">{article.summary}</p>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[var(--color-cloud)]/90 backdrop-blur-lg border-t border-[var(--color-paper-deep)] px-5 py-2.5 flex items-center justify-around">
        <button onClick={handleLike} className={`flex items-center gap-1.5 text-sm cursor-pointer transition-colors ${liked ? "text-[#FF2442]" : "text-[var(--color-ink-ghost)]"}`}>
          ...
          <span>{formatCount(liked ? article.likeCount + (article.isLiked ? 0 : 1) : article.likeCount - (article.isLiked ? 1 : 0))}</span>
        </button>
        ...
        <button onClick={handleBookmark} className={`flex items-center gap-1.5 text-sm cursor-pointer transition-colors ${bookmarked ? "text-[#F5A623]" : "text-[var(--color-ink-ghost)]"}`}>
          ...
          <span>{formatCount(bookmarked ? article.bookmarkCount + (article.isBookmarked ? 0 : 1) : article.bookmarkCount - (article.isBookmarked ? 1 : 0))}</span>
        </button>
