...
export function EditorToolbar({ editor, onInsertTopicTag, onInsertEmoji }: EditorToolbarProps) {
  ...
  return (
    <div className="flex items-center gap-1 px-4 py-2 border-b border-paper-deep/10 bg-paper-warm/30 flex-wrap shrink-0">
      ...
      {/* 标题 */}
      <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} active={editor.isActive('heading', { level: 1 })} title="大标题">
        H1
      </ToolbarButton>
      <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} active={editor.isActive('heading', { level: 2 })} title="中标题">
        H2
      </ToolbarButton>
      <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} active={editor.isActive('heading', { level: 3 })} title="小标题">
        H3
      </ToolbarButton>
      ...
      <ToolbarButton onClick={() => editor.chain().focus().toggleTaskList().run()} active={editor.isActive('taskList')} title="待办">
        ☐
      </ToolbarButton>
      ...
      <div className="relative">
        <ToolbarButton onClick={() => { setShowTags(!showTags); setShowEmoji(false); }} title="插入话题标签">
          #
        </ToolbarButton>
        {showTags && (
            ...
            <div className="flex flex-wrap gap-1">
              {SUGGESTED_TAGS.map(tag => (
                <button
                  key={tag}
                  type="button"
                  onClick={() => { onInsertTopicTag(tag); setShowTags(false); }}
      ...
      <div className="relative">
        <ToolbarButton onClick={() => { setShowEmoji(!showEmoji); setShowTags(false); }} title="插入表情">
          😊
        </ToolbarButton>
        {showEmoji && (
              ...
              {Object.keys(EMOJI_CATEGORIES).map(cat => (
                <button
                  ...
                  className={`px-2 py-0.5 text-[11px] rounded transition-colors cursor-pointer ${
                    emojiCategory === cat ? 'bg-bamboo-mist/40 text-bamboo' : 'text-ink-ghost hover:text-ink-soft'
                  }`}
                >
                  {cat}
                </button>
