...
export function ActivityTimeline({ onClose }: ActivityTimelineProps) {
  const activityLog = useStudioStore((s: { activityLog: import('../types').ActivityEntry[] }) => s.activityLog);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="absolute inset-0 bg-black/20 backdrop-blur-sm" />
      <div className="relative bg-paper rounded-2xl shadow-2xl border border-paper-deep/20 w-[420px] max-h-[500px] flex flex-col" onClick={e => e.stopPropagation()}>
