...
export function InspirationCollector({ onClose, onInsertToEditor }: InspirationCollectorProps) {
  ...
  const addInspirationDraft = useStudioStore(
    (s: { addInspirationDraft: (draft: import("../types").InspirationDraft) => void }) =>
      s.addInspirationDraft,
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div
        className="relative bg-paper rounded-2xl shadow-2xl border border-paper-deep/20 w-[480px] max-h-[600px] flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {inspirationDrafts.length === 0 ? (
            ...
            inspirationDrafts.map((draft: import("../types").InspirationDraft) => (
              <div
                key={draft.id}
                className="bg-paper-warm/30 rounded-lg p-3 border border-paper-deep/10 group"
              >
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      onInsertToEditor(draft.content);
                      onClose();
                    }}
