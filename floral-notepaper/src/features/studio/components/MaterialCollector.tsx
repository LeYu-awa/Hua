...
export function MaterialCollector({ onClose }: MaterialCollectorProps) {
  ...
  const collectedMaterials = useStudioStore(
    (s: { collectedMaterials: import("../types").CollectedMaterial[] }) => s.collectedMaterials,
  );
  const addCollectedMaterial = useStudioStore(
    (s: { addCollectedMaterial: (m: import("../types").CollectedMaterial) => void }) =>
      s.addCollectedMaterial,
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div
        className="relative bg-paper rounded-2xl shadow-2xl border border-paper-deep/20 w-[520px] max-h-[600px] flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {collectedMaterials.length === 0 ? (
            ...
            collectedMaterials.map((m: import("../types").CollectedMaterial) => (
              <div
                key={m.id}
                className="flex gap-3 p-3 bg-paper-warm/30 rounded-lg border border-paper-deep/10"
              >
                {m.coverUrl && (
                  <img
                    src={m.coverUrl}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {m.summary && (
                    <div className="text-[11px] text-ink-ghost mt-0.5 line-clamp-1">
                      {m.summary}
                    </div>
                  )}
