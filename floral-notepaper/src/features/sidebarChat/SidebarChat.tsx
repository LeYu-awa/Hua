...
const AGENT_SYSTEM_SUFFIX =
  ...
  "关于知识画布：当用户希望把画布内容「整理成架构图 / 生成架构图 / 画出系统架构 / 数据流图 / 生命周期图 / 状态机」时，调用 canvas.architecture.generate" +
  "（把用户想要的图型写进 intent，例如「把下单到发货整理成数据流图」；该工具会在画布打开生成预览，落图需用户在画布内确认）。若用户只提到笔记内容而非画布，不要使用该工具。";
