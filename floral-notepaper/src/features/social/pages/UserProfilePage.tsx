...
export function UserProfilePage({ userId, currentUserId }: UserProfilePageProps) {
  ...
  useEffect(() => {
    getUserArticles(userId).then(items => setArticles(isOwnProfile ? items : items.filter(item => item.isPublic)));
    getCategories(userId).then(setCategories);
  }, [isOwnProfile, userId]);

  const tabs: ProfileTab[] = [
    { key: 'articles' as const, label: t('profile.articles', '文章'), count: articles.length || stats?.articleCount },
    { key: 'categories' as const, label: t('profile.categories', '分类'), count: categories.length },
  ];
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'articles' && (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 p-4">
            ...
            {articles.length === 0 && (
              <div className="col-span-full text-center py-12 text-[13px] text-ink-ghost">{t('profile.noPublicArticles', '还没有公开作品')}</div>
            )}
          </div>
        )}
