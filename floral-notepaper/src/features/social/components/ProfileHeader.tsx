      ...
      <div className="relative px-8 -mt-20">
        <div className="flex items-end gap-6">
          ...
          <div className="flex-1 min-w-0 pb-2">
            <h1 className="text-[22px] font-semibold text-ink-soft">
              {profile.nickname || "未设置昵称"}
            </h1>
            <p className="text-[13px] text-ink-ghost/80 mt-0.5">
              {profile.bio || "这个人很懒，什么都没写..."}
            </p>
            <div className="flex items-center gap-4 mt-2">
              {profile.location && (
                <span className="text-[12px] text-ink-ghost/60">📍 {profile.location}</span>
              )}
              {profile.website && (
                <a
                  ...
                  className="text-[12px] text-bamboo hover:underline"
                >
                  🔗 {profile.website}
                </a>
              )}
          ...
          <div className="pb-2 shrink-0">
            {isOwnProfile ? (
              <button
                ...
                className="px-5 py-2 text-[13px] bg-paper border border-paper-deep/30 rounded-xl text-ink-soft hover:bg-paper-warm transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className={`px-5 py-2 text-[13px] rounded-xl transition-all cursor-pointer ${
                  isFollowing
                    ? "bg-paper border border-paper-deep/30 text-ink-soft hover:bg-paper-warm"
