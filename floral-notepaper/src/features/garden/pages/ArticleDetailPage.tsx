...
export function ArticleDetailPage({ userId, currentUserId, article, onBack, onRefresh }: ArticleDetailPageProps) {
  ...
  if (showEditor) {
    return (
      <ArticleEditorPage
        ...
        onSave={(saved) => { setFullArticle(saved); setShowEditor(false); }}
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-paper overflow-y-auto">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-paper-deep/10">
        <button onClick={onBack} className="flex items-center gap-1 text-[13px] text-ink-ghost hover:text-ink-soft transition-colors cursor-pointer">
          ← {t('common.back', '返回')}
        </button>
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full px-6 py-8">
          ...
          {display.tags.length > 0 && (
            <div className="flex gap-1">
              {display.tags.map(tag => (
                <span key={tag} className="px-2 py-0.5 rounded-full bg-bamboo-mist/40 text-bamboo text-[11px]">#{tag}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {/* Content */}
        <div className="mt-6 text-[15px] text-ink-soft leading-8 whitespace-pre-wrap">
          {display.content || t('garden.noContent', '暂无内容')}
        </div>
      </div>
    </div>
  );
