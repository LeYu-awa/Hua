...
export function PersonalGardenPage({ userId, onArticleClick }: PersonalGardenPageProps) {
  ...
  const recentCount = articles.filter(article => Date.now() - article.updatedAt < 1000 * 60 * 60 * 24 * 7).length;
  const growthStage = articles.length >= 12 ? t('garden.blooming', '开花期') : articles.length >= 4 ? t('garden.growing', '生长期') : t('garden.sprout', '萌芽期');

  return (
    <>
      <div className="w-56 shrink-0 border-r border-paper-deep/20 p-3 space-y-3 overflow-y-auto">
        <div>
          <div className="text-[11px] font-medium text-ink-faint/60 px-3 py-1">{t('garden.projects', '项目与文件夹')}</div>
          <button
            onClick={() => {}}
        ...
        <div className="rounded-2xl border border-paper-deep/20 bg-paper-warm/35 p-3">
          <div className="text-[11px] font-medium text-ink-faint/70 mb-2">{t('garden.growthStatus', '植物状态')}</div>
          <div className="h-24 rounded-xl bg-gradient-to-br from-bamboo-mist/70 to-paper/80 flex flex-col items-center justify-center text-center">
            ...
            <div className="text-[10px] text-ink-ghost/75 mt-0.5">{t('garden.growthHint', '持续创作会让花园更丰盛')}</div>
          </div>
        </div>
      </div>
