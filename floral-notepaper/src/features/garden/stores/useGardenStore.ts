...
export function useGardenStore(userId?: string | null) {
  ...
  const createArticle = useCallback(async (article: Partial<GardenArticle>) => {
  ...
  return {
    categories, articles, folders, loading, activeTab,
    setActiveTab, setArticles,
    loadCategories, loadPublicArticles, loadUserArticles, loadFolders,
    createCategory, createArticle, createFolder,
  };
}
