...
export function CategorySidebar({ categories, activeCategoryId, onSelectCategory, onCreateCategory }: CategorySidebarProps) {
  return (
    <div className="w-48 shrink-0 border-r border-paper-deep/20 p-3 space-y-1">
      <button
        ...
        className={`w-full text-left px-3 py-1.5 text-[13px] rounded-lg transition-colors cursor-pointer ${
          !activeCategoryId ? 'bg-bamboo-mist/60 text-bamboo font-medium' : 'text-ink-soft hover:bg-paper-warm/60'
        }`}
      >
        📋 全部
      </button>
      {categories.map(cat => (
        <button
          ...
          className={`w-full text-left px-3 py-1.5 text-[13px] rounded-lg transition-colors cursor-pointer ${
            activeCategoryId === cat.id ? 'bg-bamboo-mist/60 text-bamboo font-medium' : 'text-ink-soft hover:bg-paper-warm/60'
          }`}
        >
          {cat.icon || '📄'} {cat.name}
