//! 情绪分类器（ONNX 版）。直接移植自 LingChat `ai_service/emotion/classifier.rs`（MIT）。
//!
//! - 模型：BERT 字符级分词 + 线性分类头，最大 seq_len=128
//! - 词表：`vocab.txt`（每行一 token，id 按行号计）
//! - 标签映射：`label_mapping.json` -> `id2label` / `label2id`
//! - 推理后端：`ort`（ONNX Runtime，与 TTS 引擎共用同一依赖）
//!
//! 说明：花笺不依赖 anyhow/tracing，此处改用 `Result<T, String>` 与 `log`。
...
impl EmotionPrediction {
    pub fn passthrough(text: &str, disabled: bool) -> Self {
        Self {
...
impl EmotionClassifier {
    ...
    pub fn load<P: AsRef<Path>>(model_dir: P) -> Result<Self> {
        ...
        if !onnx_path.exists() {
            return Err(format!("ONNX 模型文件不存在: {}", onnx_path.display()));
        }
        if !vocab_path.exists() {
            return Err(format!("词表文件不存在: {}", vocab_path.display()));
        }
        if !mapping_path.exists() {
            return Err(format!("标签映射文件不存在: {}", mapping_path.display()));
        }
        ...
        let session = Session::builder()
            ...
            .map_err(|e| format!("加载 ONNX 模型失败 ({}): {e}", onnx_path.display()))?;

        // 词表：每行一个 token，行号即 id
        let vocab_text = std::fs::read_to_string(&vocab_path)
            .map_err(|e| format!("读取 vocab 失败 ({}): {e}", vocab_path.display()))?;
        ...
        let mapping_raw = std::fs::read_to_string(&mapping_path)
            .map_err(|e| format!("读取 label_mapping 失败 ({}): {e}", mapping_path.display()))?;
        let mapping: LabelMappingFile = serde_json::from_str(&mapping_raw)
            .map_err(|e| format!("解析 label_mapping.json 失败: {e}"))?;
        let id2label: HashMap<i64, String> = mapping
            ...
            .map(|(k, v)| {
                k.parse::<i64>()
                    .map(|id| (id, v))
                    .map_err(|e| format!("id2label 键不是整数: {k} ({e})"))
            })
            .collect::<Result<_>>()?;
    ...
    fn run_inference(
        ...
        let logits = outputs[0]
            ...
            .map_err(|e| format!("输出张量类型不是 f32: {e}"))?;
        let logits_slice = logits
            .as_slice()
            .ok_or_else(|| "logits 非连续布局".to_string())?;
